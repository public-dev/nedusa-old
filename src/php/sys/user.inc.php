...
class User
{
        ...
        public function login($username, $password)
        {
                ...
                {
                        //setcookie('key', $this->generateSession($row['uid']), (86400 * 365), "/");
                        //setcookie('id', $row['uid'], (86400 * 365), "/");
        ...
        {
                $sql_fetch_uname = "SELECT * FROM users WHERE uname = ? LIMIT 1";
                $sql_fetch_email = "SELECT * FROM users WHERE email = ? LIMIT 1";

                $query_uname = $this->conn->prepare($sql_fetch_uname);
                $query_email = $this->conn->prepare($sql_fetch_email);
                
                $query_uname->execute([$username]);
                $query_email->execute([$email]);

                $unu = $query_uname->fetch();
                $enu = $query_email->fetch();
